    ...
    where TProperty : notnull
{
    ...
    internal Task<TResult> EvaluateMinAsync<TResult>(
        ...
        => Task.FromResult(_evaluator.EvaluateMin<TResult>(null, selector.Compile(), filter));

    internal Task<TResult> EvaluateMaxAsync<TResult>(
        Expression<Func<T, TResult>> selector, ValiFlow<T>? filter, CancellationToken cancellationToken)
        where TResult : INumber<TResult>
        => Task.FromResult(_evaluator.EvaluateMax<TResult>(null, selector.Compile(), filter));

    internal Task<decimal> EvaluateAverageAsync<TResult>(
        Expression<Func<T, TResult>> selector, ValiFlow<T>? filter, CancellationToken cancellationToken)
        where TResult : INumber<TResult>
        => Task.FromResult(_evaluator.EvaluateAverage<TResult>(null, selector.Compile(), filter));

    internal Task<TResult> EvaluateSumAsync<TResult>(
        Expression<Func<T, TResult>> selector, ValiFlow<T>? filter, CancellationToken cancellationToken)
        where TResult : INumber<TResult>
        => Task.FromResult(_evaluator.EvaluateSum<TResult>(null, selector.Compile(), filter));
}
